...
@WebMvcTest(controllers = AssessmentController.class, excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = JwtAuthenticationFilter.class))
...
class AssessmentControllerTest {
    ...
    @MockitoBean
    private LearnerAssessmentService learnerAssessmentService;
        ...
        mockMvc
                .perform(
                        MockMvcRequestBuilders
                                .multipart("/api/courses/{courseId}/assessments/{assessmentId}", COURSE_ID,
                                        ASSESSMENT_ID)
                                .with(request -> {
    ...
    void deleteAssessment_assistantAllowed() throws Exception {
        mockMvc
                .perform(delete("/api/courses/{courseId}/assessments/{assessmentId}", COURSE_ID, ASSESSMENT_ID))
                .andExpect(status().isOk());
    ...
    void listHomework_authenticated() throws Exception {
        ...
        mockMvc
                .perform(get("/api/courses/{courseId}/assessments/homework", COURSE_ID)
    ...
    void startAssessment_learnerAllowed() throws Exception {
        AssessmentStartResponse startResponse = new AssessmentStartResponse(
                ASSESSMENT_ID, "Exam", AssessmentType.EXAM, 40, 60, AssessmentFileType.PDF, "url");
        when(learnerAssessmentService.getAssessmentForTaking(COURSE_ID, ASSESSMENT_ID, "learner@test.com")).thenReturn(startResponse);
    ...
    void submitAssessment_learnerAllowed() throws Exception {
        ...
        AssessmentSubmitResponse resp = new AssessmentSubmitResponse(
                UUID.randomUUID(), 1, 40, 0, 40, new BigDecimal("10.0"), null, List.of());
        when(learnerAssessmentService.submitAssessment(eq(COURSE_ID), eq(ASSESSMENT_ID), eq("learner@test.com"), any()))
                .thenReturn(resp);
    ...
    void listAttempts_learnerAllowed() throws Exception {
        when(learnerAssessmentService.listAttempts(eq(COURSE_ID), eq(ASSESSMENT_ID), eq("learner@test.com"),
                any(Pageable.class)))
    ...
    void getAttemptDetail_learnerAllowed() throws Exception {
        ...
        when(learnerAssessmentService.getAttemptDetail(COURSE_ID, ASSESSMENT_ID, attemptId, "learner@test.com"))
                .thenReturn(resp);

        mockMvc
                .perform(get("/api/courses/{courseId}/assessments/{assessmentId}/attempts/{attemptId}", COURSE_ID,
