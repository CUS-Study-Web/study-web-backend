...
public record UpdateAssessmentRequest(
        ...
        @Schema(description = "Max score (EXAM only)") @Min(value = 0, message = "Max score must be non-negative") Integer maxScore,
        @Schema(description = "Access tier (EXAM only)") AccessTier accessTier,
        @Schema(description = "New assessment file (PDF, DOCX, XLSX)", format = "binary") MultipartFile file,
