...
public record CreateAssessmentRequest(
        ...
        @Size(max = 500, message = "Explanation URL must not exceed 500 characters") String explanationUrl,
        @Schema(description = "Subject ID (required for HOMEWORK)") UUID subjectId,
        @Schema(description = "Duration in minutes (EXAM only)") @Min(value = 0, message = "Duration must be non-negative") Integer durationMin,
        @Schema(description = "Max score (EXAM only, default 100)") @Min(value = 0, message = "Max score must be non-negative") Integer maxScore,
        @Schema(description = "Access tier (EXAM only, default PUBLIC)") AccessTier accessTier,
        @Schema(description = "Assessment file (PDF, DOCX, XLSX)", format = "binary") @NotNull(message = "File is required") MultipartFile file,
        @Schema(description = "Answer keys as JSON array, e.g. [{\"questionNumber\":1,\"correctAnswer\":\"A\"}]") String answerKeys,
