...
public class LearnerAssessmentServiceImpl implements LearnerAssessmentService {
    ...
    public AssessmentStartResponse getAssessmentForTaking(
        ...
        String presignedUrl = assessment.getFileKey() != null ? fileService.buildFileUrl(assessment.getFileKey())
                : null;
    ...
    public Page<AssessmentAttemptResponse> listAttempts(
        ...
        return page.map(
                attempt -> {
                    int numCorrect = (int) attempt.getDetails().stream()
                            .filter(
                                    d -> {
    ...
    /**
     * Grades the student's answers against the answer keys. Returns a list of
     ...
     * CascadeType.ALL, and returns the
     * saved entity.
     */
    ...
    /**
     * Finds the user's selected answer for a specific question. Returns
